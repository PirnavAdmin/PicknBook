...
namespace PickNBook.Api.Controllers
{
    ...
    ILogger<FlightBookingsController> logger) : BaseApiController
    {
            ...
            [FromQuery] string tripType = "OneWay")
        {
            ...
            try
            {
                return await strategy.ExecuteAsync(async () =>
                {
                    ...
                    if (request.SelectedPromotionId.HasValue && !pricingBreakdown.PromotionId.HasValue)
                    {
        ...
        public async Task<IActionResult> CancelFlightBooking(int bookingId, [FromQuery] string? reason)
        {
            ...
            try
            {
                ...
                var cancelledPassengerIds = await dbContext.FlightReservationPassengers
                    .Where(x => x.FlightReservationId == bookingId)
                    .Select(x => x.Id)
                    .ToListAsync();

                await TrySendFlightCancellationNotificationsAsync(bookingId, userId!, cancelledPassengerIds, currentRefund);
        ...
        public async Task<IActionResult> CancelFlightPassengers(int bookingId, [FromBody] CancelPassengersRequestDto request)
        {
            if (!currentUserService.IsAuthenticated())
            {
                return Unauthorized("Please login to continue booking.");
            ...
            try
            {
                ...
                await TrySendFlightCancellationNotificationsAsync(bookingId, userId!, request.PassengerIds, currentRefund);
