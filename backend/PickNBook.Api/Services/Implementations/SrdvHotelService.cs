                ...
                if (markupService != null)
                {
                    ...
                    foreach (var rm in resDto.HotelRoomsDetails)
                    {
                        if (rm.Price != null)
                        {
                            ...
                            rm.B2CBasePrice = rm.Price.B2CBasePrice;
                            rm.B2CTotalPrice = rm.Price.B2CTotalPrice;
                            totalBaseFare += rm.B2CTotalPrice;
                        }
                    }
                    ...
                    if (!string.IsNullOrWhiteSpace(request.CouponCode))
                    {
                        ...
                        var couponResult = await markupService.CalculateCouponDiscountAsync(request.CouponCode, totalBaseFare, userId, false);
                        
                        if (couponResult.CouponDiscount > 0 && resDto.HotelRoomsDetails.Count > 0)
                        {
                            var firstRoom = resDto.HotelRoomsDetails[0];
                            if (firstRoom.Price != null)
                            {
                                firstRoom.Price.CouponDiscount = couponResult.CouponDiscount;
                                firstRoom.Price.B2CTotalPrice -= couponResult.CouponDiscount;
                                
                                firstRoom.B2CTotalPrice = firstRoom.Price.B2CTotalPrice;
                            }
                        }
                    }
                }
