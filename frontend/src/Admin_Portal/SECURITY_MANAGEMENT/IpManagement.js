...
export default function IpManagement({ defaultSubTab = 'all' }) {
  ...
  // Sync subtab state with route path changes (ensures sidebar updates accurately)
  useEffect(() => {
    ...
    if (path.includes('white-list-ip')) {
      ...
      setCurrentPage(1);
    } else if (path.includes('ip-management')) {
      setIpSubTab('all');
      setCurrentPage(1);
    }
  }, [location.pathname]);
    ...
    <div className="security-mgmt-container">
      ...
      <div className="sd-top-header" style={{ marginBottom: '16px' }}>
        <div className="sd-header-left">
          ...
          <p className="sd-page-subtitle">Security Management &nbsp;/&nbsp; IP Management &nbsp;/&nbsp; All IPs</p>
        </div>
        <div className="sd-header-right">
          <button className="sd-export-btn" onClick={handleOpenAddDrawer} style={{ background: '#901335', color: '#fff', border: '1px solid #901335' }}>
            ➕ Add IP Rule
          </button>
        </div>
      </div>
        ...
        {[
          ...
          <div
            ...
            onClick={() => {
              setIpSubTab(tab.key);
              setCurrentPage(1);
              if (tab.key === 'whitelist') {
                navigate('/admin/security-management/white-list-ip');
              } else if (tab.key === 'blacklist') {
                navigate('/admin/security-management/black-list-ip');
              } else {
                const path = location.pathname || '';
                if (path.includes('white-list-ip') || path.includes('black-list-ip')) {
                  navigate('/admin/security-management/ip-management');
                }
              }
            }}
          >
      ...
      <div className="sd-panel" style={{ marginTop: '12px', padding: '16px' }}>
        <div className="sd-panel-header" style={{ marginBottom: '12px' }}>
          <h3>IP Rules List <span className="sd-records-count">({totalEntries} Records)</span></h3>
          <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
            <button className="sd-table-action-btn" onClick={() => showToast('Exporting IP list CSV...')}>
              📥 Export
            </button>
            <button
