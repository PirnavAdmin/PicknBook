...
const defaultMockBusUsedCoupons = [
  {
    id: 101,
    couponCode: "SUMMER25",
...
export default function AdminBusUsedCouponListPage() {
  ...
  const [usedCoupons, setUsedCoupons] = useAdminList("bus-used-coupons", defaultMockBusUsedCoupons);
  ...
  useEffect(() => {
    async function loadData() {
      setIsLoading(true);
      try {
        const remote = await listBusUsedCoupons();
        if (remote && remote.length > 0) {
          const formatted = remote.map(r => ({
            ...
            route: r.route || "Hyderabad → Bangalore",
            createdBy: r.createdBy || "Admin"
          }));
          setUsedCoupons(formatted);
        }
      } catch (err) {
        console.error("Failed to load remote bus coupons, using mocks instead", err);
      } finally {
        setIsLoading(false);
      }
    }
  ...
  const handleExport = () => {
    ...
    link.href = url;
    link.download = `bus-used-coupons-${new Date().toISOString().slice(0, 10)}.csv`;
    link.click();
  };
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.header}>
        <AdminHeader title="B2C Bus Used Coupon List" subtitle="Manage all B2C bus coupons and offers" />
      ...
      <div style={styles.statsGrid}>
        ...
        <div style={styles.statCard}>
          <div style={styles.statInfo}>
            ...
            <span style={{ fontSize: '11px', color: '#64748B' }}>All bus coupons</span>
          </div>
          ...
          <div style={styles.sidebarPanel}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px', background: colors.slateBg, borderRadius: '12px', marginBottom: '20px' }}>
              <div>
                ...
                <div style={{ fontSize: '0.85rem', color: colors.textSecondary, marginTop: '6px' }}>Bus Booking Discount</div>
              </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', marginBottom: '24px' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.875rem' }}>
                ...
                <span style={styles.bookingTypePill}>Bus</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.875rem' }}>
