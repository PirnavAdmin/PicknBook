...
import { Check, Download, Pencil, Plus, Trash2, X, Eye, SlidersHorizontal, Tag, DollarSign, CheckCircle2, Clock, Calendar, Users } from "lucide-react";
...
function getCouponSortValue(coupon, sortBy) {
  if (sortBy === "id") return Number(coupon.id) || 0;
  if (sortBy === "value") return Number(coupon.value) || 0;
  if (sortBy === "useLimit") return Number(coupon.useLimit) || 0;
...
export default function AdminBusCouponListPage() {
  ...
  const [selectedRecord, setSelectedRecord] = useState(null); // Side Drawer
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      await updateBusCoupon(editCoupon.id, payload);
      setEditCoupon(null);
      loadCoupons();
    } catch (err) {
  ...
  const handleSaveDelete = async () => {
    ...
    try {
      await deleteBusCoupon(deleteCouponRecord.id);
  ...
  const handleExport = () => {
    ...
    const csvRows = filteredCoupons.map((c) => [
      c.id, c.couponCode, c.remark || "Bus Discount", c.cpnType, c.value, c.minBookingAmount, formatCouponDate(c.startDate), formatCouponDate(c.expiryDate), c.route || "All Routes", c.status, c.useLimit, formatCouponDateTime(c.entryDate)
    ]);
    ...
    link.href = url;
    link.download = `bus-coupons-${new Date().toISOString().slice(0, 10)}.csv`;
    link.click();
  };
    ...
    <div style={styles.container}>
      ...
      <div style={styles.header}>
        <div style={styles.titleSection}>
          <h1 style={styles.titleMain}>B2C Bus Coupon List</h1>
          <p style={styles.titleSub}>Manage all B2C bus coupons and offers</p>
        </div>
        <div style={styles.actionGroup}>
          ...
          <Link 
            to="/admin/b2c-bus/used-coupon-list"
            style={{
              padding: '10px 20px',
              borderRadius: '12px',
              fontSize: '15px',
      ...
      <div style={styles.statsGrid}>
        <div style={styles.statCard}>
          <div style={styles.statInfo}>
            ...
            <span style={styles.statSubtext}>All bus coupons</span>
          </div>
      ...
      {isFilterOpen && (
        <div style={styles.filterPanel}>
          <div style={styles.filterGrid}>
            ...
            <div style={styles.filterItem}>
              <span style={styles.filterLabel}>Applicable Routes</span>
              <select 
                value={filterRoute}
              ...
              >
                ...
                <option value="Bangalore → Vijayawada">Bangalore → Vijayawada</option>
            ...
            {isLoadingCoupons ? (
              ...
              paginatedCoupons.map((coupon) => (
                <tr key={coupon.id}>
                  ...
                  <td style={{ ...styles.td, fontWeight: 600 }}>{coupon.remark || "Bus Discount"}</td>
                  ...
                  <td style={styles.td}>
                    <div style={{ display: 'flex', gap: '8px' }}>
                      <button 
                        onClick={() => setSelectedRecord(coupon)}
                        style={{
                          width: '36px',
                          height: '36px',
                          borderRadius: '10px',
